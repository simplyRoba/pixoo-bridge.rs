...
2. **Capture startup configuration right after environment parsing**. `main()` already reads `PIXOO_BRIDGE_HEALTH_FORWARD` and `PIXOO_BASE_URL`; we will log those values (masking sensitive parts of the URL) together with the default HTTP port and retry/backoff policy baked into `PixooClient`. This gives operators a one-shot record of what the container considered its configuration.
