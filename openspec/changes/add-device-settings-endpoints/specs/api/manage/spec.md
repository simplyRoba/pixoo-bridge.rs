...
The bridge SHALL expose `PUT /manage/time/mode/{mode}` to allow configuring the device's time format. The endpoint SHALL accept `12h` or `24h` as the `{mode}` path parameter.
...
- **WHEN** a client sends `PUT /manage/time/mode/12h`
...
- **WHEN** a client sends `PUT /manage/time/mode/24h`
...
- **WHEN** a client sends `PUT /manage/time/mode/invalid`
- **THEN** the bridge returns HTTP 400 and does NOT send a command to Pixoo

### Requirement: Set temperature unit
The bridge SHALL expose `PUT /manage/weather/temperature-unit/{unit}` to allow configuring the device's temperature display unit. The endpoint SHALL accept `celsius` or `fahrenheit` as the `{unit}` path parameter.
...
- **WHEN** a client sends `PUT /manage/weather/temperature-unit/celsius`
...
- **WHEN** a client sends `PUT /manage/weather/temperature-unit/fahrenheit`
...
- **WHEN** a client sends `PUT /manage/weather/temperature-unit/kelvin`
- **THEN** the bridge returns HTTP 400 and does NOT send a command to Pixoo
