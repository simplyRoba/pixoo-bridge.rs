...
On startup the container logs the resolved configuration (health forwarding flag, sanitized Pixoo base URL, and retry/backoff policy). Unexpected Pixoo errors are logged with context; set `PIXOO_BRIDGE_LOG_LEVEL=debug` to also see notable successes like health checks or retries that eventually succeed.
